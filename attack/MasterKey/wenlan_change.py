...
if __name__ == '__main__':
    ...
    master_key = MasterKey(openai_key=OPENAI_API_KEY, generation_model="gpt-4o",
    ...
    new_jailbreak_prompt = master_key.generate_new_jailbreak_prompt(
        jailbreak_prompt)
    ...
    jailbreak_executed = master_key.execute_jailbreak_prompt(
        new_jailbreak_prompt + malicious_instruction)
    print(f"Jailbreak executed: {jailbreak_executed}")

    # Evaluate the jailbreak prompt
    jailbreak_successful = master_key.evaluate_jailbreak_prompt(
        malicious_instruction, jailbreak_executed)
