...
class Jailbroken(Attack):
    """
    llm Jailbroken
    """
    def __init__(self, logfile, target_model):
        super(Jailbroken, self).__init__(logfile, target_model)

        with open(self.logfile, 'r') as f:
            log = json.load(f)

        self.prompts = [self.create_prompt(p) for p in log['jailbroken_prompt']]
