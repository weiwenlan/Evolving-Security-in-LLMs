...
# Function to get rejected chat requests from the database
def get_rejected_requests(conn):
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, model_name, prompt_input FROM attacked_requests WHERE status = 'rejected'"
    )
...
# Main function to process rejected requests without multithreading
def process_rejected_requests():
    ...
    rejected_requests = get_rejected_requests(conn)
...
# Main function to process rejected requests with multithreading
def process_rejected_requests_multithreaded():
    conn = create_connection(db_path)
    if conn is None:
        print("Error! Cannot create database connection.")
        return
...
if __name__ == "__main__":
    process_rejected_requests()  # or process_rejected_requests_multithreaded()
